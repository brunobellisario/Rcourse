...
#The  plot is a histogram of the Turbidity values, with a normal curve superimposed.
#Looking at the gray bars, this data is skewed strongly to the right (positive skew),
#and looks more or less log-normal. The gray bars deviate noticeably from the normal curve.
qqnorm(Turbidity,
...
#The function decostand() in vegan provides some popular (and effective)
...
#QUICK EXERCISE######################################################################
#1.Create a histogram of the petal length distribution
#2.Normalize and standardize this variable
...
#In R language, there is NO function to calculate the mode.
...
#In R, max() and min() is used to find the same, unlike range() function
...
#Check if we have the package or we need to install it
#load the library
...
plotNormalHistogram(Setosa$Sepal.Length)
plotNormalHistogram(Versicolor$Sepal.Length)
plotNormalHistogram(Virginica$Sepal.Length)
#Now measure the asymmetry in data distribution
skewness(Setosa$Sepal.Length)
skewness(Versicolor$Sepal.Length)
skewness(Virginica$Sepal.Length)
kurtosis(Setosa$Sepal.Length)
kurtosis(Versicolor$Sepal.Length)
kurtosis(Virginica$Sepal.Length)
...
#QUICK EXERCISE######################################################################
#1.Measure the asymmetry of petal length for all iris species
#2.Create a multiple plot of their distributions
